...
userSchema.post<UserDocument>("deleteOne", async function (res, next) {
  try {
    await Email.findOneAndDelete({ _id: this.email }).exec();
    next();
  } catch (err: any) {
    next(err);
  }
...
userSchema.post<UserDocument[]>("deleteMany", async function (res, next) {
  try {
